...
|---|---|
| Windows, macOS, Linux (x64 / arm64) via `TelemetryDashboard.Host` | Any browser on the same machine; any browser on the same segment with `--listen network`; anything a tunnel or reverse proxy is put in front of |

That right-hand column read "Any browser: desktop, tablet, Android, iOS" for a long time and it was the largest untrue claim in this file: the console bound loopback only, in every configuration the product shipped, and no operator could change it. `TelemetryStreamingServer` took an `acceptRemoteConnections` argument that documented at length why it defaulted off, and nothing passed it — not the headless host, not the desktop shell, not one test. There was no flag and no environment variable.

The absence was deliberate rather than forgotten, and the reason was the same sentence every time: this endpoint streams live plant telemetry, replays recorded incidents and **accepts commands over its WebSocket**, and it had no authentication of any kind. Wiring the flag first would have published a plant to whatever shared the subnet.

**The decision has since been made, in two halves.** `--credential` came first — a salted PBKDF2 credential demanded on every path, the console page and the JSON endpoints and the SSE stream and the WebSocket upgrade, with `telemetry-host credential` to enrol one without a desktop. Then `--listen network`, which binds every interface and **cannot be asked for without `--credential`**: the parser refuses the pair before anything binds, and `TelemetryStreamingServer.Start` refuses it again at the socket, so the unsafe state has no construction path rather than a convention against it.

**What that buys, stated exactly.** The credential decides who may connect. Nothing decides who may read what crosses the wire: the link is plain HTTP and Basic is base64, so on the segment the password and the telemetry are both readable. `--listen network` is therefore a bench-LAN answer and is described as one — the banner says it at every launch and `/api/status` carries `reachability.encrypted: false` so a consumer is not left to infer it from the presence of `authenticated: true`.

**What was rejected.** The alternative was to permit wide binding only behind a TLS-terminating reverse proxy. A process cannot verify that a proxy is in front of it — `X-Forwarded-Proto` is written by whoever connects — so that flag's safety would have rested entirely on a sentence in a document, which is the kind of claim this codebase has executable rules against. Proxying also needs no flag: it works against the loopback binding today, unchanged, and remains the answer for a plant network or anything routed.

**What is still owed.** Confidentiality, and it is a real gap rather than a rounding error. Terminating TLS in-process is awkward here — `HttpListener` needs a `netsh` certificate binding on Windows and does not support HTTPS off it — so the honest options are an in-process switch to a different HTTP stack, or leaving TLS to the proxy it already works with. That is a product decision and is recorded rather than guessed at.
...
- **Binding.** The default is loopback and stays there; `--listen network` opens every interface and is refused without `--credential`, in the parser and again at the socket. This bullet used to say "exposing it is an operator's deliberate choice, not a default's", which read as though an operator had the choice when no flag existed. Now they do, and the choice they are actually making is which segment they trust — see the table above.
...
**Feature 13 runs all four languages for real — in the desktop shell, and nowhere else.** `FormulaScriptEngine` (expressions), `ManagedAssemblyScriptEngine` (C#/.NET in a collectible `AssemblyLoadContext`), `JavaScriptEngine` (Jint) and `PythonScriptEngine` (IronPython). Both interpreters are pure managed code — no Node, no CPython, no native binary — so a script behaves identically anywhere it is *run*. `HotReloadPluginSandbox` registers all four, which is what makes a `.js` or `.py` file execute rather than be listed as an unsupported extension.

Where it is run is the correction. This paragraph used to end "on every platform the host runs on", and the headless host does not construct the sandbox. It does not reference a script engine at all — a grep for one across `TelemetryDashboard.Host` returns nothing — and `PluginDiscovery` enumerates `*.dll` only. Outside the tests, the single production construction is `HotReloadPluginSandbox _sandbox = new()` in `UI/Dialogs/PluginSandboxDialog.xaml.cs`. So a `.js` or `.py` file dropped into a headless host's `plugins/` directory does nothing, silently, on Windows as much as on Linux.

This is the wiring ratchet's stated blind spot rather than a hole in it. The rule asks whether a type is referenced, and `HotReloadPluginSandbox` is — by a WPF dialog. "Referenced" and "reachable from the product that claims the feature" are different questions, and the second one is the one an operator is asking. Recorded rather than fixed: wiring scripts into the headless ingest path is a product decision about when they load, which hooks they may occupy and what budget they get, and inventing an answer here would be the same overclaiming this note exists to undo.
