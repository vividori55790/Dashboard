...
| A dashboard generated rather than hand-built | Half built | `Core/Services/GrafanaDashboardExport`, `/api/export/grafana`, and a setup panel in the console that hands over the scrape config and the file. Only channels that have reported get a panel. Validated against the schemastore grafana-dashboard schema and diffed field-by-field against a known-good community dashboard; **no real Grafana has imported it**, because the Docker engine does not start on this machine. Its queries are checked against what `/metrics` actually exposes by `GrafanaScrapeContractTests` -- a seam that was broken when the two halves first met |
