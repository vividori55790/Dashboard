...
| A dashboard generated rather than hand-built | Built | `Core/Services/GrafanaDashboardExport`, `/api/export/grafana`, and a setup panel that hands over the scrape config and the file. Only channels that have reported get a panel. **A real `grafana/grafana` imports it in CI** and the stored copy is read back by uid with its panels intact -- an import that stores nothing and one that stores the file both answer 200. Every query it generates is put to Prometheus's own query API and must return a series, which is the seam that broke when the two halves first met |
