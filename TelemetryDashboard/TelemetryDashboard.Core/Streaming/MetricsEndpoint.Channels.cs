...
public static partial class MetricsEndpoint
{
    ...
    private static void WriteChannels(Document document, TelemetryStreamingServer server, double nowSec)
    {
        ...
        {
            if (reading.Value is { } current) value.Sample(current, "channel", reading.Channel);
        }
        ...
        foreach (Reading reading in readings) seen.Sample(reading.NewestSec, "channel", reading.Channel);
        ...
        foreach (Reading reading in readings) appended.Sample(reading.Appended, "channel", reading.Channel);
        ...
        foreach (Reading reading in readings) evicted.Sample(reading.Evicted, "channel", reading.Channel);
        ...
        foreach (Reading reading in readings) disordered.Sample(reading.OutOfOrder, "channel", reading.Channel);
    }
