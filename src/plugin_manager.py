...
# * v2.1.0 (2026-05-29) - Antigravity: Registered Protocol Analyzer to certified plugins catalog.
...
"""
    ...
    "protocol_analyzer": {
        ...
        "name": "Protocol Analyzer Console",
        "author": "Antigravity Dev",
        "version": "1.0.0",
        "category": "디버깅/도구",
        "icon": "⚡",
        "description": "실시간 Raw HEX 패킷 스트림을 컬러풀한 태그 오버레이로 파싱하고 마킹하여 시각화해 주는 고기능 콘솔 분석기입니다.",
        "url": "https://raw.githubusercontent.com/vividori55790/Dashboard/main/plugins/protocol_analyzer.py",
        "code": '''# ======================================================================
# [FILE METADATA & VERSION TRACKING]
...
class ProtocolAnalyzerPlugin(BasePlugin):
    ...
    def on_telemetry_routed_sniff(self, subsystem_name, data):
        if self.is_paused or "hex_sensor_value" in data:
            return
        timestamp = time.strftime("%H:%M:%S")
        snippet = ", ".join([f"{k}={v}" for k, v in list(data.items())[:3]])
        if len(data) > 3:
            snippet += "..."
        html = f"<font color='#555577'>[{timestamp}]</font> <font color='#8e94a6'>[Sniff ➔ {subsystem_name}]:</font> <font color='#00ff88'>{snippet}</font>"
        self.append_html_line(html)
